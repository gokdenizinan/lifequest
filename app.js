...
const CLOUD_CONFIG_KEY = "lifequest_ai_supabase_config_v1";

let supabaseClient = null;
let currentUser = null;
let cloudSaveTimer = null;
let isApplyingCloudLoad = false;
...
function saveState(options = {}) {
  ...
  if (indicator) {
    indicator.textContent = `Saved locally ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
    indicator.classList.add("flash");
    setTimeout(() => indicator.classList.remove("flash"), 800);
  }

  if (!options.skipCloud && !isApplyingCloudLoad) {
    queueCloudSave();
  }
}
...
function getCloudConfig() {
  ...
  return {
    url: stored.url || window.LIFEQUEST_SUPABASE_URL || "",
    key: stored.key || window.LIFEQUEST_SUPABASE_PUBLISHABLE_KEY || window.LIFEQUEST_SUPABASE_ANON_KEY || ""
  };
}
...
function saveCloudConfig() {
  ...
  if (!url || !key) {
    setCloudStatus("Paste both your Supabase project URL and publishable/anon key first.", "warn");
    return;
  }

  localStorage.setItem(CLOUD_CONFIG_KEY, JSON.stringify({ url, key }));
  createSupabaseClient();
  setCloudStatus("Cloud settings saved on this browser. Sign in to start syncing.", "ok");
}

function createSupabaseClient() {
  const config = getCloudConfig();
  const canCreate = Boolean(config.url && config.key && window.supabase?.createClient);
  const urlInput = document.getElementById("supabaseUrlInput");
  const keyInput = document.getElementById("supabaseKeyInput");
  ...
  if (!window.supabase?.createClient) {
    setCloudStatus("Supabase library did not load. Check your internet connection or CDN access.", "warn");
    return false;
  }
  if (!canCreate) {
    setCloudStatus("Cloud save is off. Add your Supabase URL and publishable/anon key to enable it.", "warn");
    return false;
  }
...
async function refreshCloudUser() {
  ...
  updateAuthUi();
  if (currentUser) {
    setCloudStatus(`Signed in as ${currentUser.email}. Local changes will sync to cloud.`, "ok");
  }
}
...
function updateAuthUi() {
  ...
  if (userText) userText.textContent = signedIn ? `Signed in: ${currentUser.email}` : "Not signed in";
  ["signOutBtn", "cloudSaveBtn", "cloudLoadBtn"].forEach(id => {
...
function getAuthCredentials() {
  ...
  if (!email || !password) {
    setCloudStatus("Enter an email and password first.", "warn");
    return null;
  }
  if (password.length < 6) {
    setCloudStatus("Use a password with at least 6 characters.", "warn");
    return null;
  }
...
async function signUpWithEmail() {
  if (!supabaseClient && !createSupabaseClient()) return;
  ...
  updateAuthUi();
  setCloudStatus("Account created. If Supabase asks for email confirmation, open the confirmation email, then sign in here.", "ok");
}
...
async function signInWithEmail() {
  ...
  currentUser = data.user || null;
  updateAuthUi();
  setCloudStatus("Signed in. Loading your cloud save...", "ok");
  await loadFromCloud({ confirmOverwrite: false });
  queueCloudSave();
}
...
async function signOut() {
  ...
  currentUser = null;
  updateAuthUi();
  setCloudStatus("Signed out. Your local save remains on this browser.", "ok");
}

function queueCloudSave() {
  if (!supabaseClient || !currentUser) return;
  window.clearTimeout(cloudSaveTimer);
  cloudSaveTimer = window.setTimeout(() => saveToCloud({ silent: true }), 900);
}

async function saveToCloud({ silent = false } = {}) {
  if (!supabaseClient || !currentUser) {
    if (!silent) setCloudStatus("Sign in before saving to cloud.", "warn");
    return;
  }
  ...
  const { error } = await supabaseClient
    .from("lifequest_saves")
    .upsert(payload, { onConflict: "user_id" });

  if (error) {
    setCloudStatus(`Cloud save failed: ${error.message}`, "warn");
    return;
  }

  const time = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  setCloudStatus(`Cloud saved ${time}.`, "ok");
}

async function loadFromCloud({ confirmOverwrite = true } = {}) {
  if (!supabaseClient || !currentUser) {
    setCloudStatus("Sign in before loading from cloud.", "warn");
    return;
  }

  if (confirmOverwrite) {
    const confirmed = confirm("Load the cloud save and replace the local save in this browser?");
    if (!confirmed) return;
  }
  ...
  if (!data?.save_data) {
    setCloudStatus("No cloud save found yet. Your local save will become the first cloud save.", "ok");
    await saveToCloud({ silent: false });
    return;
  }

  isApplyingCloudLoad = true;
  state = mergeState(cloneState(defaultState), data.save_data);
  saveState({ skipCloud: true });
  isApplyingCloudLoad = false;
  renderAll();
  setCloudStatus(`Cloud save loaded. Last cloud update: ${new Date(data.updated_at).toLocaleString()}.`, "ok");
}
...
async function initCloudSave() {
  ...
  document.getElementById("cloudSaveBtn")?.addEventListener("click", () => saveToCloud({ silent: false }));
  document.getElementById("cloudLoadBtn")?.addEventListener("click", () => loadFromCloud({ confirmOverwrite: true }));
